...
function strokePropsFor(color: string) {
  return {
    stroke: color,
    strokeWidth: 2,
...
function BouquetIcon({ color }: { color: string }) {
  const strokeProps = strokePropsFor(color);
  return (
    <Svg {...svgProps}>
      ...
      <Circle cx="39" cy="23" r="2" fill={color} />
...
function DeliveryIcon({ color }: { color: string }) {
  ...
  return (
    <Svg {...svgProps}>
      ...
      <Circle cx="41" cy="17" r="2" fill={color} />
    </Svg>
  );
}

function FloralArchIcon({ color }: { color: string }) {
  const strokeProps = strokePropsFor(color);
  return (
    <Svg {...svgProps}>
      ...
      <Circle cx="43" cy="25" r="1.5" fill={color} />
...
export default function FloralServiceIcon({ name, compact = false }: FloralServiceIconProps) {
  ...
  if (name === 'bouquet-custom') icon = <BouquetIcon color={colors.primary} />;
  else if (name === 'delivery-bouquet') icon = <DeliveryIcon color={colors.primary} />;
  else if (name === 'floral-arch') icon = <FloralArchIcon color={colors.primary} />;
  else {
    icon = (
      <Ionicons
